...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    StructType(
      StructField(columnNames.head, StringType, nullable = false) :: columnNames.tail.map(name => StructField(name, DoubleType, nullable = false))
    )
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val columnAcc = (List.empty[Column], List.empty[Column], List.empty[Column])

    val classifiedColumns = columnNames.foldLeft(columnAcc) {
      (acc, name) => {
        val column = col(name)
        if (isPrimaryNeeds(name)) (column :: acc._1, acc._2, acc._3)
        else if (isWork(name)) (acc._1, column :: acc._2, acc._3)
        else if (isOther(name)) (acc._1, acc._2, column :: acc._3)
        else acc
      }
    }
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column = when($"teage".between(15, 23), "young").when($"teage".between(23, 56), "active").otherwise("elder").as("age")
